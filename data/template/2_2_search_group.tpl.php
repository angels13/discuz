...
0
|| checktplrefresh('./template/default/search/group.htm', './template/default/search/header.htm', 1531797514, '2', './data/template/2_2_search_group.tpl.php', './template/yeei_dream1', 'search/group')
|| checktplrefresh('./template/default/search/group.htm', './template/default/search/pubsearch.htm', 1531797514, '2', './data/template/2_2_search_group.tpl.php', './template/yeei_dream1', 'search/group')
|| checktplrefresh('./template/default/search/group.htm', './template/default/search/thread_list.htm', 1531797514, '2', './data/template/2_2_search_group.tpl.php', './template/yeei_dream1', 'search/group')
|| checktplrefresh('./template/default/search/group.htm', './template/default/search/group_list.htm', 1531797514, '2', './data/template/2_2_search_group.tpl.php', './template/yeei_dream1', 'search/group')
|| checktplrefresh('./template/default/search/group.htm', './template/default/search/footer.htm', 1531797514, '2', './data/template/2_2_search_group.tpl.php', './template/yeei_dream1', 'search/group')
|| checktplrefresh('./template/default/search/group.htm', './template/default/common/header_common.htm', 1531797514, '2', './data/template/2_2_search_group.tpl.php', './template/yeei_dream1', 'search/group')
;?>
...
<input type="hidden" name="srchfid" value="<?php echo $srchfid;?>" /><?php $keywordenc = $keyword ? rawurlencode($keyword) : '';?><?php if($searchid || ($_GET['adv'] && CURMODULE == 'forum')) { ?>
...
<?php } ?><?php if(!empty($_G['setting']['pluginhooks']['group_top'])) echo $_G['setting']['pluginhooks']['group_top'];?>

</form>

<?php if(!empty($searchid) && submitcheck('searchsubmit', 1)) { if($srchfid) { ?><div class="tl">
<div class="sttl mbn">
...
<div class="sttl mbn">
<h2><?php if($keyword && empty($searchstring['2'])) { ?>结果: <em>找到相关<?php echo $_G['setting']['navs']['3']['navname'];?> <?php echo $groupnum;?> 个,&nbsp;&nbsp;相关主题 <?php echo $index['num'];?> 个</em>  <?php if(empty($viewgroup) && !empty($grouplist)) { ?><a href="search.php?mod=group&amp;searchid=<?php echo $searchid;?>&amp;orderby=<?php echo $orderby;?>&amp;ascdesc=<?php echo $ascdesc;?>&amp;searchsubmit=yes&amp;viewgroup=1" target="_blank">查看<?php echo $_G['setting']['navs']['3']['navname'];?></a><?php } } elseif($viewgroup) { ?>结果: <em>找到相关<?php echo $_G['setting']['navs']['3']['navname'];?> <?php echo $groupnum;?> 个</em><?php } else { ?>结果: <em>找到相关主题 <?php echo $index['num'];?> 个</em><?php } ?></h2>
</div><?php echo adshow("search/y mtw");?><?php if($viewgroup) { if(empty($grouplist)) { ?>
<p class="emp xs2 xg2">对不起，没有找到匹配结果。</p>
<?php } else { ?>
<div class="slst pbm bbda cl"><?php if(is_array($grouplist)) foreach($grouplist as $group) { ?><dl class="xld xld_a z" style="width: 350px;">
<dd class="m"><a href="forum.php?mod=group&amp;fid=<?php echo $group['fid'];?>" target="_blank"><img src="<?php echo $group['icon'];?>" alt="" /></a></dd>
<dt><a href="forum.php?mod=group&amp;fid=<?php echo $group['fid'];?>" target="_blank"><?php echo $group['name'];?></a><?php if($group['gviewperm'] == 1) { ?>(公开)<?php } ?></dt>
<dd>成员: <?php echo $group['membernum'];?>, 主题: <?php echo $group['threads'];?></dd>
<dd>积分: <?php echo $group['commoncredits'];?>, 创建时间: <?php echo $group['dateline'];?></dd>
</dl>
<?php } ?>
</div>
<?php if(!empty($multipage)) { ?><div class="pgs cl mbm"><?php echo $multipage;?></div><?php } } } else { if(!empty($grouplist) && $page < 2) { ?>
<div class="slst pbm bbda cl"><?php if(is_array($grouplist)) foreach($grouplist as $group) { ?><dl class="xld xld_a z" style="width: 350px;">
<dd class="m"><a href="forum.php?mod=group&amp;fid=<?php echo $group['fid'];?>" target="_blank"><img src="<?php echo $group['icon'];?>" alt="" /></a></dd>
<dt><a href="forum.php?mod=group&amp;fid=<?php echo $group['fid'];?>" target="_blank"><?php echo $group['name'];?></a><?php if($group['gviewperm'] == 1) { ?>(公开)<?php } ?></dt>
<dd>成员: <?php echo $group['membernum'];?>, 主题: <?php echo $group['threads'];?></dd>
...
<div class="slst mtw">
<ul><?php if(is_array($threadlist)) foreach($threadlist as $thread) { ?><li class="pbw">
<h3 class="xs3"><a href="forum.php?mod=viewthread&amp;tid=<?php echo $thread['tid'];?>&amp;highlight=<?php echo $index['keywords'];?>" target="_blank" <?php echo $thread['highlight'];?>><?php echo $thread['subject'];?></a></h3>
<p class="xg1"><?php echo $thread['replies'];?> 个回复 - <?php echo $thread['views'];?> 次查看</p>
...
<?php if(!empty($_G['setting']['pluginhooks']['group_bottom'])) echo $_G['setting']['pluginhooks']['group_bottom'];?><?php $focusid = getfocus_rand($_G[basescript]);?><?php if($focusid !== null) { $focus = $_G['cache']['focus']['data'][$focusid];?><div class="focus" id="focus">
<div class="bm">
<div class="bm_h cl">
